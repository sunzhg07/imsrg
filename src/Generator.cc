...
Generator::Generator()
    : generator_type("white"), /* modelspace(NULL),*/ denominator_cutoff(1e-6),
      denominator_delta(0), denominator_delta_index(-1),
      denominator_partitioning(Epstein_Nesbet), only_2b_eta(false),
      use_isospin_averaging(false), only_1b_eta(false) {}

void Generator::SetDenominatorPartitioning(std::string dp) {
  if (dp == "Moller_Plesset" or dp == "Moller-Plesset" or
      dp == "MollerPlesset" or dp == "MP")
...
Operator Generator::GetHod(Operator &H) {
  for (auto sr :
       {"wegner", "white", "atan", "imaginary-time", "qtransfer-atan"}) {
    if (generator_type == sr)
      return GetHod_SingleRef(H);
  }
  for (auto sm : {"shell-model-wegner", "shell-model", "shell-model-atan",
                  "shell-model-imaginary-time"}) {
    if (generator_type == sm)
      return GetHod_ShellModel(H);
  }
  std::cout << "GetHod not implemented for generator type " << generator_type
            << "   so you get zero." << std::endl;
  return 0 * H;
}
...
void Generator::SetDenominatorDeltaOrbit(std::string orb) {
  if (orb == "all")
    ...
    std::cout << "Setting denominator delta orbit " << orb << " => "
              << H->modelspace->GetOrbitIndex(orb) << std::endl;
  }
}
...
double Generator::Get2bDenominator(int ch_bra, int ch_ket, int ibra, int iket) {
  ...
  if (denominator_partitioning == MP_isospin) {
    ...
    denominator += H->OneBody(ii, ii) + H->OneBody(jj, jj) -
                   H->OneBody(kk, kk) - H->OneBody(ll, ll);
    denominator /= 2;
  }
...
void Generator::ConstructGenerator_SingleRef_3body(
  ...
  //     std::cout << __func__ << "  looping in generator 3-body part .  Size of
  //     H3 = " << H->ThreeBodyNorm() << std::endl;
  //    for (auto a : H->modelspace->core )
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) * occnat_c *
           (1 - occnat_c)) < H->modelspace->GetOccNat3Cut())
        continue;
      size_t a = bra.p;
      size_t b = bra.q;
      size_t c = bra.r;
      ...
      for (size_t iket = 0; iket < nkets3; iket++) {
        ...
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_k *
             (1 - occnat_k)) < H->modelspace->GetOccNat3Cut())
          continue;
        size_t i = ket.p;
        size_t j = ket.q;
        size_t k = ket.r;
  ...
  //    std::cout << "Norm of Eta3 = " << std::setprecision(8) <<
  //    Eta->ThreeBodyNorm() << std::endl;
...
void Generator::ConstructGenerator_ShellModel(
  ...
  for (int ch = 0; ch < nchan; ++ch) {
    ...
    for (auto &iket : VectorUnion(tbc.GetKetIndex_cc(), tbc.GetKetIndex_vc())) {
      for (auto &ibra : VectorUnion(tbc.GetKetIndex_vv(), tbc.GetKetIndex_qv(),
                                    tbc.GetKetIndex_qq())) {
    ...
    for (auto &iket : tbc.GetKetIndex_vv()) {
      for (auto &ibra :
           VectorUnion(tbc.GetKetIndex_qv(), tbc.GetKetIndex_qq())) {
        double denominator = Get2bDenominator(ch, ibra, iket);
        ETA2(ibra, iket) = eta_func(H2(ibra, iket), denominator);
        ETA2(iket, ibra) = -ETA2(ibra, iket); // Eta needs to be antisymmetric
      }
...
void Generator::ConstructGenerator_ShellModel_3body(
  ...
  if (H->OneBodyNorm() < 1e-6) {
    std::cout << __func__
              << "  WARNING: || H1b || is zero. This may cause trouble with "
                 "the denominators."
              << std::endl;
  }
  //     std::cout << __func__ << "  looping in generator 3-body part .  Size of
...
void Generator::ConstructGenerator_ShellModel_NpNh(
  ...
  for (int ch = 0; ch < nchan; ++ch) {
    ...
    for (auto &iket : tbc.GetKetIndex_cc()) {
      Ket &ket = tbc.GetKet(iket);
      for (auto &ibra :
           VectorUnion(tbc.GetKetIndex_vc(), tbc.GetKetIndex_qc())) {
        Ket &bra = tbc.GetKet(ibra);
        if ((ket.p == bra.p) or (ket.p == bra.q) or (ket.q == bra.p) or
            (ket.q == bra.q))
          continue;
  ...
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      for (size_t iket = 0; iket < nkets3; iket++) {
        ...
        Hod.ThreeBody.SetME_pn_ch(
            ch3, ch3, ibra, iket,
            h_abcijk); // hermitian conjugate automatically gets added
...
Operator Generator::GetHod_ShellModel(Operator &H) {
  ...
  for (auto &iter : H.TwoBody.MatEl) {
    ...
    // Decouple the core
    for (auto &iket :
         VectorUnion(tbc_ket.GetKetIndex_cc(),
                     tbc_ket.GetKetIndex_vc())) // cc means core-core ('holes'
                                                // refer to the reference state)
    {
      for (auto &ibra :
           VectorUnion(tbc_bra.GetKetIndex_vv(), tbc_bra.GetKetIndex_qv(),
    ...
    {
      for (auto &ibra :
           VectorUnion(tbc_bra.GetKetIndex_qv(), tbc_bra.GetKetIndex_qq())) {
        Hod2(ibra, iket) = H2(ibra, iket);
        Hod2(iket, ibra) = H2(iket, ibra); // Eta needs to be antisymmetric
      }
    }
  ...
  // off-diagonal:   <ppp|ccc>, <ppp|vcc>, <ppp|vvc>, <qpp|vvv>  where p is v or
  // q
  //
  size_t nch3 = H.modelspace->GetNumberThreeBodyChannels();
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      for (size_t iket = 0; iket < nkets3; iket++) {
        ...
        Hod.ThreeBody.SetME_pn_ch(
            ch3, ch3, ibra, iket,
            ME_od); // hermitian conjugate automatically gets added
