...
EOM::EOM(Operator &Hs, Operator &rdm, int J2, int parity, int itz)
    : modelspace(Hs.modelspace), H(Hs), rdm(rdm), J2(J2), parity(parity),
    itz(itz) {
        eom_dims = 0;
        qv_dim = 0;
        ph_dim = 0;
...
EOM::EOM(Operator &Hs, int J2, int parity, int itz)
    : modelspace(Hs.modelspace), H(Hs), J2(J2), parity(parity), itz(itz) 
{};
...
double EOM::GetVSEOM_Overlap_multiref(Operator &H, Operator &Rd) {
    ...
    for (auto &iter : H.TwoBody.MatEl) {
                                                    ...
                                                    // refer to the reference state)
        {
            ...
            for (auto &ibra : tbc_bra.GetKetIndex_vv()) {
                ...
                //        std::cout << dbra.p <<dbra.q<<dket.p<<dket.q<<"
                //        "<<tbc_bra.J<<" "<<H2(ibra,iket) <<" "
                //        <<r2(ibra,iket)<<std::endl;
            }
        }
    }

    // std::cout << " overlap, 0b, 1b,2b : "<<ovlp <<" " << ovlp1 << " " <<ovlp2<<
    // std::endl;
