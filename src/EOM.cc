...
void EOM::ConstructConfigs()
{
    ...
    if(qv_dim >0)qv_end=qv_start+qv_dim-1;
    ...
    if(ph_dim >0)ph_end = ph_start+ph_dim-1;
    ...
    if(ppvv_dim>0)ppvv_end=ppvv_start+ppvv_dim-1;
    ...
    if(pphv_dim>0)pphv_end=pphv_start+pphv_dim-1;
    ...
    if(pphh_dim>0)pphh_end=pphh_start+pphh_dim-1;
...
void EOM::ConstructNormMatrix()
{
    ...
    if(pphv_dim!=0){
        ...
        void EOM::ConstructProjectMatrix()
        {
            ...
            // first we do pphh, it have no coupling to other diagrams, and no coupling bettwen channels
           std::cout << " svd for pphh : " << std::endl;

           for (index_t ich=0; ich <number_channels; ich++)
            {
                if(nconf_pphh_ch[ich]==0)continue;
                std::vector<int> coupled_idx;
                for(index_t i=pphh_start; i<= pphh_end; i++){
                    if(ich==eom_confs.at(i)[2])coupled_idx.push_back(i);}
                if(coupled_idx.size()==0)continue;
            ...
            for (index_t i = ppvv_start; i<= ppvv_end; i++)
            {
                ...
                if(op.cvq != 1)continue;//p should be valence
                if(oq.cvq == 1)continue; // q should be exclude
                coupled_idx.push_back(i);
            }

            std::cout << " svd for vqvv and pv a : " << qv_start<<" "<<qv_end << std::endl;

            for (index_t i = qv_start; i<= qv_end; i++)
                coupled_idx.push_back(i);
            std::cout << " svd for vqvv and pv b : " << std::endl;

            if(coupled_idx.size()!=0) block_svd(coupled_idx);
            ...
            for (index_t ich=0; ich <number_channels; ich++)
            {
                ...
                if(nconf_ppvv_ch[ich]==0)continue;
                coupled_idx.clear();

                for(index_t i=ppvv_start; i<= ppvv_end; i++){
                    if(ich!=eom_confs.at(i)[2])continue;
                    ...
                    Orbit &op = modelspace->GetOrbit(p);
                    Orbit &oq = modelspace->GetOrbit(q);
                    if(op.cvq == 1)continue;//p should be exclude
                    if(oq.cvq == 1)continue; // q should be exclude
                    coupled_idx.push_back(i);
                }
            ...
            for (index_t ich=0; ich <number_channels; ich++)
            {
                ...
                if(nconf_pphv_ch[ich]==0)continue;
                coupled_idx.clear();
                for(index_t i=pphv_start; i<= pphv_end; i++){
                    if(ich!=eom_confs.at(i)[2])continue;
                    size_t ibra = eom_confs.at(i)[0];
