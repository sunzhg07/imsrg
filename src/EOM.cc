...
std::vector<std::tuple<size_t,size_t,size_t,double>> EOM::ThreeBody_Diagram_Entries_Internal(size_t a, size_t b, size_t c, size_t d, size_t e,
  ...
  const int jab_seed = int(std::lround(j0));
  const int jde_seed = int(std::lround(j2));

  const double ja = 0.5 * oa.j2;
  const double jb = 0.5 * ob.j2;
  const double jc = 0.5 * oc.j2;
  const double jd = 0.5 * od.j2;
  const double je = 0.5 * oe.j2;
  const double jf = 0.5 * of.j2;

  // The sparse seeds are X(ab;dg;Jab_seed)=1 and Y(cg;ef;Jde_seed)=1.
  // If either seed cannot live in its required two-body channel there is no diagram.
  int ch_Xb = modelspace->GetTwoBodyChannelIndex(jab_seed, (oa.l + ob.l) % 2, (oa.tz2 + ob.tz2) / 2);
  int ch_Xk = modelspace->GetTwoBodyChannelIndex(jab_seed, (od.l + og.l) % 2, (od.tz2 + og.tz2) / 2);
  int ch_Yb = modelspace->GetTwoBodyChannelIndex(jde_seed, (oc.l + og.l) % 2, (oc.tz2 + og.tz2) / 2);
  int ch_Yk = modelspace->GetTwoBodyChannelIndex(jde_seed, (oe.l + of.l) % 2, (oe.tz2 + of.tz2) / 2);
  if (ch_Xb < 0 || ch_Xk < 0 || ch_Yb < 0 || ch_Yk < 0)
    return result;

  int x_ib = modelspace->GetTwoBodyChannel(ch_Xb).GetLocalIndex(std::min(a, b), std::max(a, b));
  int x_ik = modelspace->GetTwoBodyChannel(ch_Xk).GetLocalIndex(std::min(d, g), std::max(d, g));
  int y_ib = modelspace->GetTwoBodyChannel(ch_Yb).GetLocalIndex(std::min(c, g), std::max(c, g));
  int y_ik = modelspace->GetTwoBodyChannel(ch_Yk).GetLocalIndex(std::min(e, f), std::max(e, f));
  if (x_ib < 0 || x_ik < 0 || y_ib < 0 || y_ik < 0)
    return result;

  // X and Y each contain one antisymmetrized seed matrix element plus its
  // transpose. For the lookup we only need orbit identity, the pair J values,
  // and the hermitian or antihermitian sign.
  auto single_seed_tbme = [&](size_t seed_bra_1, size_t seed_bra_2,
                              size_t seed_ket_1, size_t seed_ket_2,
                              int J_seed_bra, int J_seed_ket, int herm_sign,
                              int ch_bra, int ch_ket,
                              size_t I1, size_t I2, size_t I3, size_t I4) -> double {
    int J_bra = modelspace->GetTwoBodyChannel(ch_bra).J;
    int J_ket = modelspace->GetTwoBodyChannel(ch_ket).J;

    size_t bra_lo = std::min(I1, I2);
    size_t bra_hi = std::max(I1, I2);
    size_t ket_lo = std::min(I3, I4);
    size_t ket_hi = std::max(I3, I4);
    size_t seed_bra_lo = std::min(seed_bra_1, seed_bra_2);
    size_t seed_bra_hi = std::max(seed_bra_1, seed_bra_2);
    size_t seed_ket_lo = std::min(seed_ket_1, seed_ket_2);
    size_t seed_ket_hi = std::max(seed_ket_1, seed_ket_2);

    double phase = 1.0;
    if (I1 > I2) {
      Orbit &o1 = modelspace->GetOrbit(I1);
      Orbit &o2 = modelspace->GetOrbit(I2);
      phase *= -modelspace->phase((o1.j2 + o2.j2 - 2 * J_bra) / 2);
    }
    if (I3 > I4) {
      Orbit &o3 = modelspace->GetOrbit(I3);
      Orbit &o4 = modelspace->GetOrbit(I4);
      phase *= -modelspace->phase((o3.j2 + o4.j2 - 2 * J_ket) / 2);
    }

    bool is_direct = (J_bra == J_seed_bra && J_ket == J_seed_ket
                   && bra_lo == seed_bra_lo && bra_hi == seed_bra_hi
                   && ket_lo == seed_ket_lo && ket_hi == seed_ket_hi);
    if (is_direct)
      return phase;

    bool is_transpose = (J_bra == J_seed_ket && J_ket == J_seed_bra
                      && bra_lo == seed_ket_lo && bra_hi == seed_ket_hi
                      && ket_lo == seed_bra_lo && ket_hi == seed_bra_hi);
    if (is_transpose)
      return phase * double(herm_sign);

    return 0.0;
  };

  const std::array<Permutation, 3> perms = {Permutation::ABC, Permutation::CBA, Permutation::ACB};

  auto permute = [](Permutation perm, size_t in1, size_t in2, size_t in3) {
    switch (perm) {
      case Permutation::ABC: return std::array<size_t, 3>{in1, in2, in3};
      case Permutation::CBA: return std::array<size_t, 3>{in3, in2, in1};
      case Permutation::ACB: return std::array<size_t, 3>{in1, in3, in2};
      default:               return std::array<size_t, 3>{in1, in2, in3};
    }
  };

  auto permutation_sign = [](Permutation perm) {
    return (perm == Permutation::ABC) ? 1 : -1;
  };

  auto coupling_range = [](Permutation perm, const Orbit &ox, const Orbit &oy, const Orbit &oz,
                           int pair_coupling, int twoJ) {
    if (perm == Permutation::ABC)
      return std::pair<int, int>{pair_coupling, pair_coupling};

    return std::pair<int, int>{
      std::max(std::abs(ox.j2 - oy.j2), std::abs(twoJ - oz.j2)) / 2,
      std::min(ox.j2 + oy.j2, twoJ + oz.j2) / 2,
    };
  };

  auto pair_exists_in_channel = [&](int channel, size_t left, size_t right) {
    return channel >= 0
        && modelspace->GetTwoBodyChannel(channel).GetLocalIndex(std::min(left, right), std::max(left, right)) >= 0;
  };

  auto antisymmetry_phase = [&](const Orbit &oleft, const Orbit &oright, int pair_J, size_t left, size_t right) {
    return (left > right) ? -modelspace->phase((oleft.j2 + oright.j2 - 2 * pair_J) / 2) : 1.0;
  };

  auto recouple = [&](Permutation perm,
                      double jx, double jy, double jz,
                      int Jpair_new, int Jpair_old, int twoJ) -> double {
    const double total_J = 0.5 * twoJ;
    if (std::abs(int(jx - jy)) > Jpair_old || int(jx + jy) < Jpair_old)
      return 0.0;
    if (std::abs(int(jz - total_J)) > Jpair_old || int(jz + total_J) < Jpair_old)
      return 0.0;
    switch (perm) {
      case Permutation::ABC:
        return (Jpair_old == Jpair_new) ? 1.0 : 0.0;
      case Permutation::CBA:
        return -std::sqrt((2.0 * Jpair_new + 1.0) * (2.0 * Jpair_old + 1.0))
               * modelspace->GetSixJ(jx, jy, Jpair_old, jz, total_J, Jpair_new);
      case Permutation::ACB:
        return modelspace->phase(jy + jz + Jpair_new - Jpair_old)
               * std::sqrt((2.0 * Jpair_new + 1.0) * (2.0 * Jpair_old + 1.0))
               * modelspace->GetSixJ(jy, jx, Jpair_old, jz, total_J, Jpair_new);
      default:
        return 0.0;
    }
  };

  auto sixj_value = [&](const Orbit &o3, const Orbit &o6, const Orbit &oint, int Jab_prime, int Jde_prime, int twoJ) -> double {
    if (twoJ <= 2 * modelspace->GetEmax() + 1) {
      return modelspace->GetCachedSixJ(o3.j2, twoJ, Jab_prime, o6.j2, oint.j2, Jde_prime);
    }

    double j3 = 0.5 * o3.j2;
    double j6 = 0.5 * o6.j2;
    double jint = 0.5 * oint.j2;
    double total_J = 0.5 * twoJ;
    if (oint.j2 < twoJ) {
      return modelspace->GetSixJ(j6, jint, Jab_prime, j3, total_J, Jde_prime);
    }
    return modelspace->GetSixJ(j6, total_J, Jde_prime, j3, jint, Jab_prime);
  };

  modelspace->PreCalculateSixJ();

  // The intermediate one-body line can only be one of the external or seed orbits.
  std::vector<size_t> candidates = {a, b, c, d, e, f, g};
  std::sort(candidates.begin(), candidates.end());
  candidates.erase(std::unique(candidates.begin(), candidates.end()), candidates.end());

  int bra_twoJ_min = std::abs(2 * jab_seed - oc.j2);
  int bra_twoJ_max = 2 * jab_seed + oc.j2;
  int ket_twoJ_min = std::abs(2 * jde_seed - od.j2);
  ...
  for (int twoJ = std::max(bra_twoJ_min, ket_twoJ_min);
    ...
    int Jab_max = std::min(oa.j2 + ob.j2, twoJ + oc.j2) / 2;
    int Jde_min = std::max(std::abs(od.j2 - oe.j2), std::abs(twoJ - of.j2)) / 2;
    int Jde_max = std::min(od.j2 + oe.j2, twoJ + of.j2) / 2;
    ...
    for (int Jab = Jab_min; Jab <= Jab_max; ++Jab) {
      ...
      for (int Jde = Jde_min; Jde <= Jde_max; ++Jde) {
        ...
        for (auto perm_abc : perms) {
          auto bra_perm = permute(perm_abc, a, b, c);
          size_t I1 = bra_perm[0];
          size_t I2 = bra_perm[1];
          size_t I3 = bra_perm[2];
          Orbit &o1 = modelspace->GetOrbit(I1);
          Orbit &o2 = modelspace->GetOrbit(I2);
          Orbit &o3 = modelspace->GetOrbit(I3);

          std::pair<int, int> Jab_prime_range = coupling_range(perm_abc, o1, o2, o3, Jab, twoJ);
          int Jab_prime_min = Jab_prime_range.first;
          int Jab_prime_max = Jab_prime_range.second;

          for (int Jab_prime = Jab_prime_min; Jab_prime <= Jab_prime_max; ++Jab_prime) {
            double rec_abc = recouple(perm_abc, ja, jb, jc, Jab_prime, Jab, twoJ)
                           * permutation_sign(perm_abc);
            if (std::abs(rec_abc) < 1e-14)
              continue;
            if (I1 == I2)
              rec_abc *= SQRT2;

            int ch12 = modelspace->GetTwoBodyChannelIndex(Jab_prime, (o1.l + o2.l) % 2, (o1.tz2 + o2.tz2) / 2);
            if (!pair_exists_in_channel(ch12, I1, I2))
              continue;
            double phase_12 = antisymmetry_phase(o1, o2, Jab_prime, I1, I2);

            for (auto perm_def : perms) {
              auto ket_perm = permute(perm_def, d, e, f);
              size_t I4 = ket_perm[0];
              size_t I5 = ket_perm[1];
              size_t I6 = ket_perm[2];
              Orbit &o4 = modelspace->GetOrbit(I4);
              Orbit &o5 = modelspace->GetOrbit(I5);
              Orbit &o6 = modelspace->GetOrbit(I6);

              std::pair<int, int> Jde_prime_range = coupling_range(perm_def, o4, o5, o6, Jde, twoJ);
              int Jde_prime_min = Jde_prime_range.first;
              int Jde_prime_max = Jde_prime_range.second;

              // The commutator sums over both allowed parity sectors and proton/neutron character
              // of the intermediate one-body line.
              for (int parity_a : {0, 1}) {
                for (int tz2a : {-1, 1}) {
                  bool good_126a = ((o1.l + o2.l + o6.l + parity_a) % 2 == 0)
                                 && ((o1.tz2 + o2.tz2 - o6.tz2 - tz2a) / 2 == 0);
                  bool good_3a45 = ((o4.l + o5.l + o3.l + parity_a) % 2 == 0)
                                 && ((o3.tz2 + tz2a - o4.tz2 - o5.tz2) / 2 == 0);
                  if (!(good_126a && good_3a45))
                    continue;

                  for (int Jde_prime = Jde_prime_min; Jde_prime <= Jde_prime_max; ++Jde_prime) {
                    double rec_def = recouple(perm_def, jd, je, jf, Jde_prime, Jde, twoJ)
                                   * permutation_sign(perm_def);
                    if (std::abs(rec_def) < 1e-14)
                      continue;
                    if (I4 == I5)
                      rec_def *= SQRT2;

                    int ch45 = modelspace->GetTwoBodyChannelIndex(Jde_prime, (o4.l + o5.l) % 2, (o4.tz2 + o5.tz2) / 2);
                    int ch3a = modelspace->GetTwoBodyChannelIndex(Jde_prime, (o3.l + parity_a) % 2, (o3.tz2 + tz2a) / 2);
                    if (ch3a < 0 || !pair_exists_in_channel(ch45, I4, I5))
                      continue;
                    double phase_45 = antisymmetry_phase(o4, o5, Jde_prime, I4, I5);

                    double prefactor = rec_abc * rec_def
                                     * std::sqrt((2.0 * Jab_prime + 1.0) * (2.0 * Jde_prime + 1.0))
                                     * phase_12 * phase_45;
                    if (std::abs(prefactor) < 1e-14)
                      continue;

                    int j2a_min = std::max(std::abs(o6.j2 - 2 * Jab_prime), std::abs(o3.j2 - 2 * Jde_prime));
                    int j2a_max = std::min(o6.j2 + 2 * Jab_prime, o3.j2 + 2 * Jde_prime);

                    for (size_t orb_a : candidates) {
                      Orbit &oint = modelspace->GetOrbit(orb_a);
                      if (oint.tz2 != tz2a || oint.l % 2 != parity_a)
                        continue;
                      if (oint.j2 < j2a_min || oint.j2 > j2a_max)
                        continue;

                      int ch6a = modelspace->GetTwoBodyChannelIndex(Jab_prime, (o6.l + oint.l) % 2, (o6.tz2 + oint.tz2) / 2);
                      if (ch6a < 0)
                        continue;

                      double sixj = sixj_value(o3, o6, oint, Jab_prime, Jde_prime, twoJ);
...
// Compute the comm223ss diagram for fixed 3B bra/ket with sparse seed operators.
// X^{jab}_{sa sb, sf sg} = 1,  Y^{jde}_{sg sc, sd se} = 1  (all other elements zero).
// Returns mat(nJ1, nJ2); row r -> J1 = J1min_out+r, col c -> J2 = J2min_out+c.
arma::mat EOM::threebody_diagram_comm(
...
{
  Orbit &oi=modelspace->GetOrbit(i), &oj=modelspace->GetOrbit(j), &ok=modelspace->GetOrbit(k);
  Orbit &ol=modelspace->GetOrbit(l), &om=modelspace->GetOrbit(m), &on=modelspace->GetOrbit(n);
  double ji=0.5*oi.j2, jj=0.5*oj.j2, jk=0.5*ok.j2;
  double jl=0.5*ol.j2, jm=0.5*om.j2, jn=0.5*on.j2;

  // Valid J1 range: couple (i,j) to total twoJ with third particle k
  int J1min = std::max(std::abs(oi.j2-oj.j2), std::abs(twoJ-ok.j2)) / 2;
  int J1max = std::min(oi.j2+oj.j2, twoJ+ok.j2) / 2;
  // Valid J2 range: couple (l,m) to total twoJ with third particle n
  int J2min = std::max(std::abs(ol.j2-om.j2), std::abs(twoJ-on.j2)) / 2;
  int J2max = std::min(ol.j2+om.j2, twoJ+on.j2) / 2;

  J1min_out = J1min;
  J2min_out = J2min;

  if (J1max < J1min || J2max < J2min)
    return arma::mat();

  int nrows = J1max - J1min + 1;
  int ncols = J2max - J2min + 1;
  arma::mat result(nrows, ncols, arma::fill::zeros);

  // ---- Setup sparse getters for X and Y ----
  // X^{jab}_{sa sb, sf sg} = 1
  int ch_Xb = modelspace->GetTwoBodyChannelIndex(jab,
      (modelspace->GetOrbit(sa).l+modelspace->GetOrbit(sb).l)%2,
      (modelspace->GetOrbit(sa).tz2+modelspace->GetOrbit(sb).tz2)/2);
  int ch_Xk = modelspace->GetTwoBodyChannelIndex(jab,
      (modelspace->GetOrbit(sf).l+modelspace->GetOrbit(sg).l)%2,
      (modelspace->GetOrbit(sf).tz2+modelspace->GetOrbit(sg).tz2)/2);
  // Y^{jde}_{sg sc, sd se} = 1
  int ch_Yb = modelspace->GetTwoBodyChannelIndex(jde,
      (modelspace->GetOrbit(sg).l+modelspace->GetOrbit(sc).l)%2,
      (modelspace->GetOrbit(sg).tz2+modelspace->GetOrbit(sc).tz2)/2);
  int ch_Yk = modelspace->GetTwoBodyChannelIndex(jde,
      (modelspace->GetOrbit(sd).l+modelspace->GetOrbit(se).l)%2,
      (modelspace->GetOrbit(sd).tz2+modelspace->GetOrbit(se).tz2)/2);

  if (ch_Xb<0 || ch_Xk<0 || ch_Yb<0 || ch_Yk<0)
    return result;

  int x_ib = modelspace->GetTwoBodyChannel(ch_Xb).GetLocalIndex(std::min(sa,sb), std::max(sa,sb));
  int x_ik = modelspace->GetTwoBodyChannel(ch_Xk).GetLocalIndex(std::min(sf,sg), std::max(sf,sg));
  int y_ib = modelspace->GetTwoBodyChannel(ch_Yb).GetLocalIndex(std::min(sg,sc), std::max(sg,sc));
  int y_ik = modelspace->GetTwoBodyChannel(ch_Yk).GetLocalIndex(std::min(sd,se), std::max(sd,se));

  if (x_ib<0 || x_ik<0 || y_ib<0 || y_ik<0)
    return result;

  // Returns the TBME value applying antisymmetry phases for non-canonical ordering,
  // and hermitian conjugate (multiplied by herm) when bra/ket channels are swapped.
  auto make_getter = [&](int chb, int chk, int ib_ref, int ik_ref, int J_ref,
                         int herm)
  {
    return [=](int ch_bra, int ch_ket, size_t I1, size_t I2, size_t I3, size_t I4) -> double {
      int ib = modelspace->GetTwoBodyChannel(ch_bra).GetLocalIndex(std::min((int)I1,(int)I2), std::max((int)I1,(int)I2));
      int ik = modelspace->GetTwoBodyChannel(ch_ket).GetLocalIndex(std::min((int)I3,(int)I4), std::max((int)I3,(int)I4));
      if (ib < 0 || ik < 0) return 0.0;

      auto stored_value = [&](int ch_row, int ch_col, int row_ind, int col_ind) -> double {
        if (ch_row == chb && ch_col == chk && row_ind == ib_ref && col_ind == ik_ref) return 1.0;
        if (chb == chk && ch_row == chb && ch_col == chk && row_ind == ik_ref && col_ind == ib_ref)
          return (ib_ref == ik_ref) ? 1.0 : double(herm);
        return 0.0;
      };

      double phase = 1.0;
      if (I1 > I2) phase *= modelspace->GetTwoBodyChannel(ch_bra).GetKet(ib).Phase(J_ref);
      if (I3 > I4) phase *= modelspace->GetTwoBodyChannel(ch_ket).GetKet(ik).Phase(J_ref);

      if (ch_bra > ch_ket)
      {
        return double(herm) * phase * modelspace->phase(modelspace->GetTwoBodyChannel(ch_bra).J
                                                         - modelspace->GetTwoBodyChannel(ch_ket).J)
             * stored_value(ch_ket, ch_bra, ik, ib);
      }

      return phase * stored_value(ch_bra, ch_ket, ib, ik);
    };
  };

  auto get_X = make_getter(ch_Xb, ch_Xk, x_ib, x_ik, jab, herm_X);
  auto get_Y = make_getter(ch_Yb, ch_Yk, y_ib, y_ik, jde, herm_Y);

  // ---- Permutation helpers (only ABC, CBA, ACB as in comm223ss) ----
  const std::array<ThreeBodyStorage::Permutation,3> index_perms = {
    ThreeBodyStorage::ABC, ThreeBodyStorage::CBA, ThreeBodyStorage::ACB};

  auto do_permute = [](ThreeBodyStorage::Permutation perm,
                       size_t a_in, size_t b_in, size_t c_in,
                       size_t &a_out, size_t &b_out, size_t &c_out) {
    switch(perm) {
      case ThreeBodyStorage::ABC: a_out=a_in; b_out=b_in; c_out=c_in; break;
      case ThreeBodyStorage::CBA: a_out=c_in; b_out=b_in; c_out=a_in; break;
      case ThreeBodyStorage::ACB: a_out=a_in; b_out=c_in; c_out=b_in; break;
      default:                    a_out=a_in; b_out=b_in; c_out=c_in; break;
    }
  };

  auto perm_phase = [](ThreeBodyStorage::Permutation perm) -> int {
    return (perm==ThreeBodyStorage::ABC || perm==ThreeBodyStorage::BCA ||
            perm==ThreeBodyStorage::CAB) ? 1 : -1;
  };

  // RecouplingCoefficient for the three permutation cases (ABC, CBA, ACB only)
  auto recouple = [&](ThreeBodyStorage::Permutation perm,
                      double ja, double jb, double jc,
                      int Jab_new, int Jab, int twoJ_) -> double {
    if (std::abs(int(ja-jb)) > Jab || int(ja+jb) < Jab) return 0.0;
    if (std::abs(int(jc - twoJ_/2.)) > Jab || int(jc + twoJ_/2.) < Jab) return 0.0;
    switch(perm) {
      case ThreeBodyStorage::ABC:
        return (Jab == Jab_new) ? 1.0 : 0.0;
      case ThreeBodyStorage::CBA:
        return -sqrt((2.*Jab_new+1)*(2.*Jab+1))
               * modelspace->GetSixJ(ja, jb, Jab, jc, twoJ_/2., Jab_new);
      case ThreeBodyStorage::ACB:
        return modelspace->phase(jb + jc + Jab_new - Jab)
               * sqrt((2.*Jab_new+1)*(2.*Jab+1))
               * modelspace->GetSixJ(jb, ja, Jab, jc, twoJ_/2., Jab_new);
      default: return 0.0;
    }
  };
  ...
  // Candidate intermediate orbits: only orbits relevant to X and Y seeds
  // x_126a nonzero -> {I6,orb_a} in {sf,sg}; y_3a45 nonzero -> {I3,orb_a} in {sg,sc}
  // Intersection gives candidates {sa,sb,sc,sd,se,sf,sg}; getter filters further.
  std::vector<size_t> candidates = {sa, sb, sc, sd, se, sf, sg};
  std::sort(candidates.begin(), candidates.end());
  candidates.erase(std::unique(candidates.begin(), candidates.end()), candidates.end());

  // ---- Main loops over (J1, J2) ----
  for (int J1 = J1min; J1 <= J1max; J1++)
  {
    // Antisymmetry: |ii;J1> vanishes for odd J1 (fermions)
    if (i==j && J1%2 != 0) continue;

    for (int J2 = J2min; J2 <= J2max; J2++)
    {
      if (l==m && J2%2 != 0) continue;
      ...
      {
        size_t I1, I2, I3;
        do_permute(perm_ijk, i,j,k, I1,I2,I3);
        Orbit &o1=modelspace->GetOrbit(I1), &o2=modelspace->GetOrbit(I2), &o3=modelspace->GetOrbit(I3);
        double j3 = 0.5*o3.j2;

        // J1' range: for ABC it is fixed to J1; for others it is summed
        int J1p_min = J1, J1p_max = J1;
        ...
        {
          double rec_ijk = recouple(perm_ijk, ji,jj,jk, J1p, J1, twoJ)
                         * perm_phase(perm_ijk);
          if (std::abs(rec_ijk) < 1e-14) continue;
          if (I1==I2) rec_ijk *= SQRT2;

          int ch12 = modelspace->GetTwoBodyChannelIndex(J1p, (o1.l+o2.l)%2, (o1.tz2+o2.tz2)/2);
          if (ch12 < 0) continue;
          if (modelspace->GetTwoBodyChannel(ch12).GetLocalIndex(std::min(I1,I2), std::max(I1,I2)) < 0)
            continue;
          // Antisymmetry phase when I1 > I2 (non-canonical bra order)
          double phase_12 = (I1>I2) ? -modelspace->phase((o1.j2+o2.j2-2*J1p)/2) : 1.0;

          for (auto perm_lmn : index_perms)
          {
            size_t I4, I5, I6;
            do_permute(perm_lmn, l,m,n, I4,I5,I6);
            Orbit &o4=modelspace->GetOrbit(I4), &o5=modelspace->GetOrbit(I5), &o6=modelspace->GetOrbit(I6);
            double j6 = 0.5*o6.j2;

            // J2' range: for ABC fixed to J2; for others summed
            int J2p_min = J2, J2p_max = J2;
            ...
            for (int parity_a : {0,1})
            {
              for (int tz2a : {-1,1})
              {
                // Parity and isospin conservation at each vertex
                bool good_126a = ((o1.l+o2.l+o6.l+parity_a)%2 == 0)
                               && ((o1.tz2+o2.tz2-o6.tz2-tz2a)/2 == 0);
                bool good_3a45 = ((o4.l+o5.l+o3.l+parity_a)%2 == 0)
                               && ((o3.tz2+tz2a-o4.tz2-o5.tz2)/2 == 0);
                if (!(good_126a && good_3a45)) continue;

                for (int J2p = J2p_min; J2p <= J2p_max; J2p++)
                {
                  double rec_lmn = recouple(perm_lmn, jl,jm,jn, J2p, J2, twoJ)
                                 * perm_phase(perm_lmn);
                  if (std::abs(rec_lmn) < 1e-14) continue;
                  if (I4==I5) rec_lmn *= SQRT2;

                  int ch45 = modelspace->GetTwoBodyChannelIndex(J2p, (o4.l+o5.l)%2, (o4.tz2+o5.tz2)/2);
                  int ch3a = modelspace->GetTwoBodyChannelIndex(J2p, (o3.l+parity_a)%2, (o3.tz2+tz2a)/2);
                  if (ch45<0 || ch3a<0) continue;
                  if (modelspace->GetTwoBodyChannel(ch45).GetLocalIndex(std::min(I4,I5), std::max(I4,I5)) < 0)
                    continue;
                  // Antisymmetry phase when I4 > I5
                  double phase_45 = (I4>I5) ? -modelspace->phase((o4.j2+o5.j2-2*J2p)/2) : 1.0;

                  double hat_factor = sqrt((2.*J1p+1)*(2.*J2p+1));
                  // Triangle constraint on intermediate j_a from both 6j arguments
                  int j2a_min = std::max(std::abs(o6.j2-2*J1p), std::abs(o3.j2-2*J2p));
                  int j2a_max = std::min(o6.j2+2*J1p, o3.j2+2*J2p);
                  double prefactor = rec_ijk * rec_lmn * hat_factor * phase_12 * phase_45;

                  if (std::abs(prefactor) < 1e-14) continue;

                  // Sum over candidate intermediate orbits
                  for (size_t orb_a : candidates)
                  {
                    Orbit &oa = modelspace->GetOrbit(orb_a);
                    if (oa.tz2 != tz2a)         continue;
                    if (oa.l%2 != parity_a)     continue;
                    if (oa.j2 < j2a_min || oa.j2 > j2a_max) continue;

                    int ch6a = modelspace->GetTwoBodyChannelIndex(J1p, (o6.l+oa.l)%2, (o6.tz2+oa.tz2)/2);
                    if (ch6a < 0) continue;

                    // Commutator 6j: { j_{I3}  J   J1' ; j_{I6}  j_a  J2' }
                    double ja_val = 0.5*oa.j2;
                    double sixj;
                    if (twoJ <= 2*modelspace->GetEmax()+1)
                      sixj = modelspace->GetCachedSixJ(o3.j2, twoJ, J1p, o6.j2, oa.j2, J2p);
                    else
                      sixj = (oa.j2 < twoJ)
                           ? modelspace->GetSixJ(j6, ja_val, J1p, j3, 0.5*twoJ, J2p)
                           : modelspace->GetSixJ(j6, 0.5*twoJ, J2p, j3, ja_val, J1p);

                    // Normalization for equal-orbit pairs (phase_12/phase_45 cancel as ±1^2=1;
                    // SQRT2 is the real content for |aa;J> states)
                    double phase_6a = phase_12;
                    double phase_3a = phase_45;
                    if (I6   == orb_a) phase_6a *= SQRT2;
                    if (I3   == orb_a) phase_3a *= SQRT2;

                    double x_126a = get_X(ch12, ch6a, I1, I2, I6,    orb_a);
                    double y_126a = get_Y(ch12, ch6a, I1, I2, I6,    orb_a);
                    double x_3a45 = get_X(ch3a, ch45, I3, orb_a, I4, I5);
                    double y_3a45 = get_Y(ch3a, ch45, I3, orb_a, I4, I5);

                    double contrib = prefactor * sixj * phase_6a * phase_3a
