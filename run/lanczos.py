...
def lanczos_proc(
    ...
    for j in range(max_iter):
        ...
        if j > state_want and j % 4 == 0:
            e, v = np.linalg.eigh(
                hall[0 : j + 1, 0 : j + 1]
            )  # eigh returns sorted real eigenvalues
    ...
    for j in range(max_iter - 1):
        ...
        if abs(bj) < restart_tol:
            if restart_gen is None:
                print(
                    f"arnoldi: exact breakdown at step {j+1}, no restart_gen provided"
                )
                break
            # --- restart: inject a new random direction orthogonal to existing subspace ---
            print(
                f"arnoldi: breakdown at step {j+1} (bj={bj:.2e}), restarting with new direction"
            )
