...
print("benchmarking tensor eta comm223_232st against comm223st -> comm232st")
print(
...
def comm223_232st_chain_reference(eta, gamma, out):
    ...
    Commutator.comm232st(eta, t3, out)
    return t3


for irepeat in range(nrepeat):
    z_chain = zero_tensor_like(gamma)
    t0 = time.perf_counter()
    t3 = comm223_232st_chain_reference(eta, gamma, z_chain)
    t_chain = time.perf_counter() - t0

    z_cpp_chain = zero_tensor_like(gamma)
    t0 = time.perf_counter()
    ReferenceImplementations.comm223_232st(eta, gamma, z_cpp_chain)
    t_cpp_chain = time.perf_counter() - t0

    z_public = zero_tensor_like(gamma)
    t_public = None
    public_error = None
    t0 = time.perf_counter()
    try:
        Commutator.FactorizedDoubleCommutator.comm223_232st(eta, gamma, z_public)
