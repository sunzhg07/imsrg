...
print("benchmarking tensor eta comm223_132st against comm223st -> comm132st")
print(
...
def comm223_132st_chain_reference(eta, gamma, out):
    ...
    Commutator.comm132st(eta, t3, out)
    return t3

for irepeat in range(nrepeat):
    z_chain = zero_scalar_like(gamma)
    t0 = time.perf_counter()
    t3 = comm223_132st_chain_reference(eta, gamma, z_chain)
    t_chain = time.perf_counter() - t0

    z_cpp_chain = zero_scalar_like(gamma)
    t0 = time.perf_counter()
    ReferenceImplementations.comm223_132st(eta, gamma, z_cpp_chain)
    t_cpp_chain = time.perf_counter() - t0

    z_factorized = zero_scalar_like(gamma)
    t_factorized = None
    factorized_error = None
    t0 = time.perf_counter()
    try:
        Commutator.FactorizedDoubleCommutator.comm223_132st(eta, gamma, z_factorized)
        t_factorized = time.perf_counter() - t0
    except RuntimeError as err:
        factorized_error = str(err)

    zdiff_cpp_chain = z_cpp_chain - z_chain
    zdiff_factorized = z_factorized - z_chain
    rel_cpp_chain = zdiff_cpp_chain.Norm() / max(z_chain.Norm(), 1.0e-30)
    rel_factorized = zdiff_factorized.Norm() / max(z_chain.Norm(), 1.0e-30)

    print(f"\nrepeat {irepeat + 1}/{nrepeat}")
    print(f"python chain time:    {t_chain:.6f} s")
    print(f"c++ chain time:       {t_cpp_chain:.6f} s")
    if t_factorized is not None:
        print(f"public implementation time: {t_factorized:.6f} s")
    else:
    ...
    print_norms("factorized-chain", zdiff_factorized)
    print(f"cpp-python relative Norm: {rel_cpp_chain:.12e}")
    print(f"factorized-chain relative Norm: {rel_factorized:.12e}")
    if factorized_error is not None:
        print(f"public disable reason: {factorized_error}")
