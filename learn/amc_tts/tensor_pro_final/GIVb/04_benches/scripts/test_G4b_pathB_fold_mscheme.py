#!/usr/bin/env python3
"""Γ^{IV_b} full Z ≡ m at any λ via Path B χ + analyze fold.

Pipeline (no Factorized RC — that dual is λ=0 only so far):
  1) Path B χ̄ = Γ̄·(occ⊙Ω̄) → invPlus → χ_J  (locked ≡ m ≡ AMC all λ)
  2) Unpack χ_J → m (WE)
  3) Analyze fold: W = Σ_ab (χ_aibk Ω_jbla − χ_akbi Ω_jalb)
  4) Z = (1−P_ij)(1−P_kl) W
  5) lock Z(m) ≡ m-scheme gold
...
print(
    ...
    f"  Path B χ → analyze fold → (1−P)^2 ≡ m  (no TTS, no PandyaRC)"
)
...
if maxd < tol and n1 == len(hits):
    print(f"PASS — Path B fold ≡ m (λ={lam})")
    sys.exit(0)
print("FAIL")
sys.exit(1)
